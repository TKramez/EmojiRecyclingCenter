...
class App {
  ...
  getEmojiTotalValue(emojiIndex) {
    ...
    const size = this.blockSize;
    const value = {r: 0, g: 0, b: 0, black: 0};
    for (let x = 0; x < this.canvas.width; x += size) {
      for (let y = 0; y < this.canvas.height; y += size) {
        const i = x + y * this.canvas.width;
        let r = data[i * 4 + 0];
  ...
  drawEmojiRank() {
    ...
    this.emojiList.forEach( (e, i) => {
      ...
      const cx = (this.mousex - this.canvas.width / 2) * size;
      const cy = (this.mousey - this.canvas.width / 2) * size;
      //const cx = 0;
      //const cy = 0;
      const hsl = this.rgbToHsl(value.r * 255 / 350000, value.g * 255 / 350000, value.b * 255 / 350000);
      //const r = hsl.s / 100; 
      const r = (value.r + value.g + value.b) / (350000 * 3);
      const x = cx + (this.canvas.width / 2) + (size * 200) * Math.cos(hsl.h * Math.PI * 2 / 360) * r;
      const y = cy + (this.canvas.width / 2) + (size * 200) * Math.sin(hsl.h * Math.PI * 2 / 360) * r;
